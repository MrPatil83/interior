...
const Testimonials = ({ cards }) => {
  ...
  return (
    <section className=" laptop:mx-24 my-10 h-[80vh]  px-24  pt-9  pb-[25rem]   laptop:h-[40rem] flex flex-col justify-between rounded-br-full bg-stone-900 text-white">
      ...
      initial="hidden"
      whileInView={"visible"}
       className=" poppins-regular text-5xl mt-4 text-zinc-400">
        Esteemed Clients
      </motion.h1>
          ...
          return (
            <div
              ...
              className={classNames(
                "text-white w-[24rem] h-[12rem] flex justify-center items-center gap-4 phone:gap-5 tablet:gap-7 laptop:gap-9 rounded-[3rem] -mt-[rem] border border-gray-300 bg-transperant transition-all duration-500 ease-in-out",
                {
                  "absolute z-20 transform scale-110 opacity-100 -translate-x-1/2 translate-y-16 phone:translate-y-28 tablet:translate-y-16 laptop:translate-y-0":
                    isCurrent,
                  "absolute z-10 opacity-70 transform scale-95 translate-y-36 phone:translate-y-52 tablet:translate-y-44 laptop:translate-y-32 -translate-x-[98%]":
                    isPrev,
                  "absolute z-10 opacity-70 transform scale-95 translate-y-36 phone:translate-y-52 tablet:translate-y-44 laptop:translate-y-32 translate-x-[8%]":
                    isNext,
            ...
            >
              ...
              />
              <div className="flex flex-col gap-2 phone:gap-1 tablet:gap-3 laptop:gap-5 max-w-[75%]">
                <h1 className="poppins-regular text-sm phone:text-base tablet:text-xl laptop:text-3xl">
                  {card.name}
                </h1>
                <div className="text-xs poppins-regular phone:text-sm tablet:text-base laptop:text-lg">
                  {card.add}
                </div>
                <div className="flex gap-2 text-sm tablet:text-lg laptop:text-2xl">
                  {stars}
                </div>
                <p className="text-xs poppins-extralight text-slate-400 phone:text-sm tablet:text-sm laptop:text-base">
                  {card.quote}
                </p>
              </div>
            </div>
